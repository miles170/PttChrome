...
export function u2b(it) {
  ...
  for (var i = 0; i < it.length; ++i) {
    ...
    if (hi || lo)
      data += String.fromCharCode(hi) + String.fromCharCode(lo); // Not a big5 char
    else data += "\xFF\xFD";
  }
  return data;
}
...
export function parseStatusRow(str) {
  var regex = new RegExp(
    /  瀏覽 第 (\d{1,3})(?:\/(\d{1,3}))? 頁 *\( *(\d{1,3})%\)  目前顯示: 第 0*(\d+)~0*(\d+) 行 *(?:\(y\)回應)?(?:\(X\/?%\)推文)?(?:\(h\)說明)? *\(←\/?q?\)離開 /g,
  );
...
export function parseListRow(str) {
  var regex = new RegExp(
    /\[\d{1,2}\/\d{1,2} +星期. +\d{1,2}:\d{1,2}\] .+ 線上\d+人, 我是\w+ +\[呼叫器\](?:關閉|打開) /g,
  );
