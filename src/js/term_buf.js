...
TermBuf.prototype = {
  ...
  uriRegEx:
    /((ftp|http|https|telnet):\/\/([A-Za-z0-9_]+:{0,1}[A-Za-z0-9_]*@)?([A-Za-z0-9_#!:.?+=&%@!\-\/\$\^,;|*~'()]+)(:[0-9]+)?(\/|\/([A-Za-z0-9_#!:.?+=&%@!\-\/]))?)|(pid:\/\/(\d{1,10}))/gi,
  ...
  setPageState: function () {
    ...
    if (this.isUnicolor(0, 0, 29) && this.isUnicolor(0, cols - 20, cols - 10)) {
      ...
      if (
        ...
        this.isUnicolor(2, 0, cols - 10) &&
        !this.isLineEmpty(1) &&
        (this.cur_x < 19 || this.cur_y == lastRowNum)
      ) {
        //console.log('pageState = 2 (LIST)');
        this.pageState = 2; // LIST
      }
    } else if (
      this.isUnicolor(lastRowNum, 28, 53) &&
      this.cur_y == lastRowNum &&
      this.cur_x == cols - 1
    ) {
      //console.log('pageState = 5 (PASS)');
      this.pageState = 5; // some ansi drawing screen to pass
    ...
    switch (this.pageState) {
      ...
      case 2: //LIST
        if (trow > 2 && trow < lastRowNum) {
          ...
          else if (tcol > cols - 5) this.mouseCursor = 13;
      ...
      case 3: //READING
        if (trow == lastRowNum) {
          if (tcol < 2)
            //]
            this.mouseCursor = 12;
          else if (tcol > cols - 5) this.mouseCursor = 14;
          else this.mouseCursor = 5;
        } else if (trow === 0) {
          if (tcol < 2)
            //=
