...
export const App = function () {
  ...
  window.addEventListener("beforeunload", (e) => {
    if (this.conn && this.conn.isConnected && this.buf.pageState != 0) {
      e.preventDefault();
      return "You are currently connected. Are you sure?";
    }
  });
...
App.prototype.onClose = function () {
  ...
  ReactDOM.render(<ConnectionAlert onDismiss={onDismiss} />, container);
...
App.prototype.getWindowInnerBounds = function () {
  var width =
    document.documentElement.clientWidth - this.view.bbsViewMargin * 2;
  var height =
    document.documentElement.clientHeight - this.view.bbsViewMargin * 2;
  var bounds = {
    width: width,
    height: height,
  };
    ...
    case 6:
      if (this.buf.nowHighlight != -1) {
        ...
        if (this.buf.cur_y > this.buf.nowHighlight) {
          ...
          for (var i = 0; i < count; ++i) sendstr += "\x1b[A"; //Arrow Up
        } else if (this.buf.cur_y < this.buf.nowHighlight) {
          var count = this.buf.nowHighlight - this.buf.cur_y;
          for (var i = 0; i < count; ++i) sendstr += "\x1b[B"; //Arrow Down
        }
        sendstr += "\r";
  ...
  try {
    switch (name) {
      case "useMouseBrowsing":
        ...
        this.CmdHandler.setAttribute(
          "useMouseBrowsing",
          useMouseBrowsing ? "1" : "0",
        );
...
App.prototype.checkClass = function (cn) {
  return (
    cn.indexOf("closeSI") >= 0 ||
    cn.indexOf("EPbtn") >= 0 ||
    cn.indexOf("closePP") >= 0 ||
    cn.indexOf("picturePreview") >= 0 ||
    cn.indexOf("drag") >= 0 ||
    cn.indexOf("floatWindowClientArea") >= 0 ||
    cn.indexOf("WinBtn") >= 0 ||
    cn.indexOf("sBtn") >= 0 ||
    cn.indexOf("nonspan") >= 0 ||
    cn.indexOf("nomouse_command") >= 0
  );
};
    ...
    if (window.getSelection().isCollapsed) {
      ...
      if (this.buf.useMouseBrowsing) {
        ...
        if (e.target.className)
          if (this.checkClass(e.target.className)) doMouseCommand = false;
        if (e.target.tagName)
          if (e.target.tagName.indexOf("menuitem") >= 0) doMouseCommand = false;
        if (skipMouseClick) {
...
App.prototype.mouse_down = function (e) {
  ...
  if (e.button === 0) {
    ...
    var onbbsarea = true;
    if (e.target.className)
      if (this.checkClass(e.target.className)) onbbsarea = false;
    if (e.target.tagName)
      if (e.target.tagName.indexOf("menuitem") >= 0) onbbsarea = false;
  } else if (e.button == 2) {
  ...
  if (e.button === 0 || e.button == 2) {
    ...
    if (window.getSelection().isCollapsed) {
      ...
      if (e.button === 0) {
        ...
        if (e.target.className)
          if (this.checkClass(e.target.className)) preventDefault = false;
        if (e.target.tagName)
          if (e.target.tagName.indexOf("menuitem") >= 0) preventDefault = false;
...
App.prototype.setBBSCmd = function setBBSCmd(cmd) {
  switch (cmd) {
    ...
    case "doArrowDown":
      if (this.view.useEasyReadingMode && this.buf.startedEasyReading) {
        if (
          this.view.mainDisplay.scrollTop >=
    ...
    case "doPageUp":
      if (this.view.useEasyReadingMode && this.buf.startedEasyReading) {
        this.view.mainDisplay.scrollTop -=
          this.view.chh * this.easyReading._turnPageLines;
      } else {
    ...
    case "previousThread":
      if (this.view.useEasyReadingMode && this.buf.startedEasyReading) {
        ...
        this.buf.pageState == 2 ||
        this.buf.pageState == 3 ||
        this.buf.pageState == 4
      ) {
        this.conn.send("[");
      }
      break;
